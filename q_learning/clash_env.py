...
class ClashRoyaleEnv(gym.Env):
    def __init__(self):
        ...
        # OBSERVATION SPACE: Extracted game state vector instead of raw pixels.
        # [elixir (0-1), blue_health (0-1), red_health (0-1)]
        # Much faster to train than 84x84 CNN input — use MlpPolicy instead of CnnPolicy.
    ...
    def step(self, action):
            ...
            elixir = sd.read_elixir_bar(raw_rgb)
            info["elixir"] = elixir
            ...
            # Build obs from extracted state: [elixir, blue_health, red_health]
            blue_health, red_health = sd.check_tower_health(raw_rgb, "both")
            obs = np.array([elixir / 10.0, blue_health, red_health], dtype=np.float32)
    ...
    def reset(self, seed=None):
        ...
        # Cycle through tap locations that cover: OK/continue buttons, chest dismissal,
        # crown chest, level up, pass rewards, and the Battle button.
        tap_sequence = [
            (750, 2250),  # OK / continue (post-match banner)
            (725, 2250),  # OK double-tap
            (750, 1400),  # center of screen (chest / popup dismiss)
            (750, 1800),  # lower center (some reward screens)
            (750, 2000),  # Battle button (main menu)
            (750, 1200),  # upper center (crown chest / pass rewards)
        ]

        for attempt in range(60):  # ~2 min max before giving up
            raw_rgb = sd.get_screen_rgb()
            if sd.is_in_match(raw_rgb):
                print(f"In match! (attempt {attempt})")
                break

            tap = tap_sequence[attempt % len(tap_sequence)]
            print(f"Not in match, tapping {tap}...")
            sd.device.shell(f"input tap {tap[0]} {tap[1]}")
            time.sleep(2)
        else:
            print("WARNING: Could not get into a match after 60 attempts.")
    ...
    def _execute_adb_tap(self, action):
        ...
        sd.device.shell(f"input tap {arena_x} {arena_y}") # Tap the arena to deploy the cardit train