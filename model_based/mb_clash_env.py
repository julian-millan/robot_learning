...
import mb_screen_detection as sd 
...
class ClashRoyaleEnv(gym.Env):
    ...
    def step(self, action):
            ...
            # only calculate ongoing reward if the game is still in progress and we have elixir to work with, 
            # otherwise we might be calculating deltas during the end game screen which could be noisy and not meaningful    
            if not done and not elix_zero: 
                ...
                second_red_p_tower = red_health_list[1]
                red_king_tower = red_health_list[2]
                # blue towers
                first_blue_p_tower = blue_health_list[0]
                second_blue_p_tower = blue_health_list[1]
                blue_king_tower = blue_health_list[2]
                ...
                # now combine all the tower healths into one for the rest of our reward shaping (health difference)
                blue_health = ((first_blue_p_tower+second_blue_p_tower)/3.6) + blue_king_tower*1.6/3.6 # use our old estimation to recall the total health
                red_health = ((first_red_p_tower+second_red_p_tower)/3.6) + red_king_tower*1.6/3.6
    ...
    def _execute_adb_tap(self, action):
        ...
        # action 1..576 -> 4 cards x (16*9) placement cells
        action_idx = action - 1 # Shift down by 1 to make it zero-indexed
        cells_per_card = self.zones # 16*9 = 144
        card_idx = action_idx // cells_per_card # Determine which card to play (0-3)
        cell_idx = action_idx % cells_per_card # Determine which cell in the grid to place the card (0-143)
